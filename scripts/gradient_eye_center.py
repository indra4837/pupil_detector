...
def image_reader(filename):
    ...
    cv2.circle(img_cp, center, 10, (0, 255, 0), 4)
    cv2.circle(img_cp, center, 100, (0, 255, 0), 4)
    toc = time.time()
    img_cp = fps_overlay(img_cp, 1/(toc-tic))
    cv2.imshow('final', img_cp)
    key = cv2.waitKey(1)
    if key == ord('q'):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_file',
                        type=str,
                        dest='input_file',
                        default='/home/indra/Documents/Telemedc/pupil_center_indra/assets/sample.mkv',
                        help='Path to input file (image or video)')
    
    args = parser.parse_args()

    if args.input_format:
        video_reader(args.input_file)
    else:
        image_reader(args.input_file)